...
Knob = function (input, ui) {
    ...
    container.style.cssText =
        'position: relative; width:' + settings.width + 'px;' + 'height:' + settings.height + 'px;';
...
Knob.prototype = {
    ...
    _handleMove: function (onMove, onEnd) {
        ...
        this.centerY =
            Math.floor(this.container.getBoundingClientRect().top) + document.body.scrollTop + this.settings.height / 2;
...
Ui.Scale.prototype.dial = function () {
    ...
    if (!this.options.labels) {
        ...
        for (var i = 0; i < this.options.labels.length; i++) {
            ...
            var text = new Ui.El.Text(label, this.width / 2 - 2.5, this.height / 2 - this.options.radius, 5, 5);
            this.el.append(text);
            text.rotate(this.startAngle + i * step, this.width / 2, this.height / 2);
